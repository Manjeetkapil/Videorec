...
int record() {
	...
	ifstream fin;
	fin.open("./recorded/read.txt", ios::app | ios::in);
	string line;
	getline(fin,line);
    ...
    int count=0;
    ofstream fio;
	fio.open("./recorded/read.txt", ios::trunc | ios::out);
    ...
    for (;;) {
		...
		GaussianBlur(channels[2], channel_blurred_with_5x5_kernel, Size(5,5), 0);//channels[2] is red channel and applying gaussian blur on that channel
        count++;
