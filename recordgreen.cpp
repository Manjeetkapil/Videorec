...
//only green coloured video recording with gaussian blur

int recordgreen(string login) {
	Mat frame,R;
    VideoCapture vcap(0);
    if (!vcap.isOpened()) {
    ...
    ifstream fin;
    fin.open("./greenrecord/read.txt", ios::app | ios::in);
    string line;
    getline(fin,line);
    bool isColor = (frame.type() == CV_8UC3);
    string path = "./greenrecord/";
    path.append(login);
    path.append(line);
    string format = ".mp4";
    path.append(format);
    ...
    int count=0;
    ofstream fio;
    fio.open("./greenrecord/read.txt", ios::trunc | ios::out);
